...
|---|---|---|---|---|---|---|
| latteMCPclient | — | Not started | — | — | — | No code exists yet to reconstruct; see note below |

`latteAPI` reached `Confirmed` on 2026-08-22 (Phase 1 implementation, Track B) and `latteMCP`
reached `Confirmed` on 2026-08-22 (Phase 2 implementation, Track B); both have been dropped from
this table per the convention above — see `docs/00-index.md` for their status and
`docs/modules/latteAPI/` / `docs/modules/latteMCP/` for their docs. `latteMCPclient` shows
`Not started` here only because there is no *code* to scan yet — its `docs/modules/` content
already exists as a forward spec authored via Track B's docs-first flow (`CLAUDE.md` rule 6:
write the doc, then implement), not via Track A reconstruction. See `discovery_plan.md` for the
full picture.
